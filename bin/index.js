...
function getFrontBack() {
    inquirer.prompt([
        {
            type: 'list',
            message: 'Select the wanted Front-end/Back-end',
            name: 'frontBack',
            loop: false,
            default: 'react',
            choices: [
...
async function createProjectsFolders() {
    ...
    if (allData.frontBack !== 'none') {
        if (allData.frontBack === 'flask') {
            ...
            // Create the .env
            fs.copyFileSync(`${ownPath}/my-react-app/.env.example`, `${ownPath}/my-react-app/.env`);
        }
            ...
            // Create the .env
            fs.copyFileSync(`${ownPath}/my-vue-app/.env.example`, `${ownPath}/my-vue-app/.env`);
        }
    }
...
async function installDependency(packageName, previous = '') {
    ...
    const installInterval = setInterval(() => {
        i++;
        ui.updateBottomBar(`Installing ${packageName} ${loader[i % loader.length]}`);
    }, 300);
...
function createDockerCompose() {
    ...
    if (allData.frontBack !== 'none') {
        if (allData.frontBack === 'react') {
            content.services.web = {
                ...
                image: `${allData.name}-web`,
                container_name: `${allData.name}-web`,
                ports: [
                    '8080:8080',
                ],
                volumes: [
                    './my-react-app:/app/web',
                ],
                networks: [
                    'back-network',
                ],
    ...
    printMessage(['Thank\'s for using docker-tool', '', 'You can now run the following command to start the project:', 'docker-compose up --build']);
}
